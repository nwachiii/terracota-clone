...
import wallet from '../../../../images/icons/mainstone-wallet-card.svg'
import SelectCard from './SelectCard';
import CustomizeRecurring from './CustomizeRecurring';
import { Text } from '@chakra-ui/react';

const RecurringModal = ({ equity, recurringModal, refetch }) => {
  const autoDebitFrequencyFromAsset = equity?.payment_plan?.payment_frequency?.toString()?.toUpperCase() || null
  ...
  const methods = [
    {
      ...
      source: 'card',
      desc: (
        <Text color='#606060' fontSize={{ base: '11px', md: '14px' }}>
          Your card will be charged on your due date and the next billing cycle is
          <Text as='span' fontWeight={700} fontSize={{ base: '11px', md: '14px' }} font> {new Date(startDate).toDateString()}</Text>
        </Text>
      ),
      img: creditCard.src,
    },
