...
const SavedFilters = ({ filterDataExtracted, filtersLoading, refetch, isOpen, onClose, applySelectedFilter }) => {
  ...
  return (
    <Modal autoFocus={false} isOpen={isOpen} onClose={onClose} isCentered blockScrollOnMount>
      <ModalOverlay />
      <ModalContent
        px={{ base: '20px', md: '40px' }} bg='card_bg'
        py='35px' minW={{ base: 'auto', md: '900px' }} minH='520px'
          ...
          <>
            ...
            <VStack spacing='25px' height='45vh' css={customScrollbarStyles} overflowY={'scroll'} mt={{ base: '10px', md: '15px' }} px='5px'>
              {filterDataExtracted.map((filter, i) => (
                <Flex
                  gap={{ base: '20px', md: '50px' }} key={i} borderRadius={'2px'}
                  justify={"space-between"} px={{ base: '10px', md: "20px" }} py={{ base: '10px', md: "15px" }}
                  border={"0.5px solid #747474"} w={"100%"} align={"center"} cursor="pointer"
                >
                  <Flex
            ...
            <Button
              ...
              bg={"primary"} h={"63px"} borderRadius={"2px"}
            >
              Remove all Filters
            </Button>
          </>
