...
const FilterMobile = ({ isOpen, onClose, onFilterChange }) => {
  ...
  const getQueryString = () => {
    ...
    queryString.append(
      'payment_plan_is_available',
      isPaymentPlanOn ? 'True' : 'False'
    );
  ...
  return (
    <>
        ...
        <DrawerContent bg='background' px='18px' py='25px' minW='100vw'>
          ...
          >
            ...
            {Boolean(filterDataExtracted.length) && (
              <Text color='text' onClick={handleOpenModal} fontSize={'14px'} fontWeight={400} textDecoration={'underline'}>View saved filters</Text>
            )}
          </Flex>

          <Box mt='30px' overflowY={'scroll'} h='90vh' w='full'>
            <InputGroup w='full' borderRadius={'8px'} border={'1px solid #E4E4E4 !important'} boxShadow={'0px 2.675px 5.647px 0px rgba(142, 151, 158, 0.15)'}>
              <InputLeftAddon bg={'transparent'} px={'10px'} border={'none'}>
                <Image w='30px' h='auto' alt='next_image' src={appCurrentTheme === LIGHT ? searchIcon.src : searchIcon_light.src} />
              </InputLeftAddon>
              <Input
                placeholder='Search by Location'
                w='full' py={'10px'} pl='0px' h={'100%'}
            ...
            <Flex direction='row' justify='center' align='center' gap='10px'>
              <Button
                ...
                isLoading={isLoading} maxW={{ base: 'unset', md: '112px' }}
                onClick={handleResetFilter} w='full'
              >
                Reset
              </Button>
              <Button
                ...
                bg='primary'
                isLoading={isLoading} maxW={{ base: 'unset', md: '112px' }}
                onClick={handleApplyFilter} w='full'
              >
                Apply Filter
              </Button>
            </Flex>
            <Button
              color='primary' leftIcon={<Image w='14px' h='14px' src={saveFilterIcon.src} />}
              bg='primaryFilterOpacity' mt='16px'
              isLoading={isLoading} maxW={{ base: 'unset', md: '112px' }}
              onClick={handleSave} w='full'
            >
              Save
            </Button>
          </Box>
      ...
      <SavedFiltersMobile
        ...
        fetchedData={fetchedData} refetch={refetch}
