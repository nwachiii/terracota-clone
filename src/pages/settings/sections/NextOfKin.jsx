...
import {Button, FormInput, FormSelect, FormTextarea, Spinner, UploadProfilePicture} from '../../../ui-lib';
...
const Profile = () => {
	...
	const next_of_kinQuery = useQuery(['getSettingsData', 'profile'], () => getSettingsData({next_of_kin: true}), {
		onSuccess: (res) => {
			formik.setValues({
					...
					<SimpleGrid mt={{base: '10px', lg: '40px'}} columns={{base: 1, lg: 3}} spacing='19px' justifyContent={'space-between'}>
						...
						<GridItem colSpan={{base: 1, lg: 2}} mb={{base: '7px', lg: '30px'}} p='24px' border='1px solid #EAECF0' borderRadius={'2px'} boxShadow={'0px 1px 3px 0px rgba(16, 24, 40, 0.10), 0px 1px 2px 0px rgba(16, 24, 40, 0.06)'}>
							<SimpleGrid columns={{base: 1, lg: 2}} spacing='19px'>
								<FormInput type='text' label='Full Name' onChange={formik.handleChange('first_name')} value={`${formik.values.first_name}${formik.values.middle_name}${formik.values.last_name}`} placeholder='Enter full name' disabled={formik.values.first_name?.length > 0} />
